...
//        $currentIp      = $this->requestStack->getCurrentRequest()->getClientIp();
        ...
        $response_ = (new Client())->request('GET', $this->CDN["ip_info"] . $currentIp, [
            'decode_content' => false
        ]);
        $status = $response_->getStatusCode(); // 200
        $headerType = $response_->getHeaderLine('content-type'); // 'application/json; charset=utf8'
        $data['data'] = json_decode($response_->getBody()->getContents(), false, 512, JSON_THROW_ON_ERROR);// '{"id": 1420053, "name": "guzzle", ...}'
        $data['ip'] = $currentIp;
